...
test "cross-backend: fibonacci structure is equivalent" {
    ...
    // All should have base case check
    ...
    // All should have recursive calls
    try helpers.expectContains(results.c.output, "fib(n - 1)");
    try helpers.expectContains(results.js.output, "fib(n - 1)");
    try helpers.expectContains(results.erlang.output, "fib(N - 1)");
}
